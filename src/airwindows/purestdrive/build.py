#!/usr/bin/env python3
"""Build PURESTDR.ZDL from purestdrive.c + manifest.json."""
...
def main() -> None:
    ...
    src_c   = HERE / "purestdrive.c"
    obj     = HERE / "purestdrive.obj"
    out_zdl = ROOT / "dist" / f"{manifest['effect_name']}.ZDL"
    out_zdl.parent.mkdir(exist_ok=True)

    print(f"[purestdrive] compiling {src_c.name} → {obj.name}")
    subprocess.run(
