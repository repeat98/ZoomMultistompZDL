#!/usr/bin/env python3
"""Build StatePing.ZDL from stateping.c + manifest.json."""
...
def main() -> None:
    ...
    cfg = LinkerConfig(
        ...
        screen_image=make_airwindows_tape_screen("StatePing", ""),
        audio_nop=manifest.get("audio_nop", False),
    )
    link(cfg)

    print(f"\n[stateping] done -> {out_zdl}")
