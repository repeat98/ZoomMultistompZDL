#!/usr/bin/env python3
"""Build fixed-threshold DescSize ZDL variants from descsize.c + manifest.json."""
...
def main() -> None:
    ...
    write_param_header(manifest, HERE / "descsize_params.h", "DESCSIZE")

    src_c = HERE / "descsize.c"
    out_dir = ROOT / "dist"
    ...
    for variant in manifest["variants"]:
        ...
        threshold = int(variant["threshold_bytes"])
        ...
        print(
            f"[descsize] compiling {src_c.name} -> {obj.name} "
        ...
        subprocess.run(
            [
                str(CL6X),
                *CFLAGS,
                f"--define=DESCSIZE_AUDIO_FUNC={audio_func_name}",
                f"--define=DESCSIZE_THRESHOLD_BYTES={threshold}u",
                "-c",
                str(src_c),
                f"--output_file={obj}",
            ],
